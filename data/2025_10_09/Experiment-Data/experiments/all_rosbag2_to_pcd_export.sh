#!/bin/bash
# Parallel rosbag -> pcd exporter.
# Env vars:
#   PARALLEL_EXPS  : how many experiments to process concurrently (default 3)
#   WORKERS_PER_EXP: per-experiment worker processes (default: cores/PARALLEL_EXPS)
...
process_one() {
    ...
    mapfile -t rosbag_dirs < <(find "$exp" -maxdepth 1 -type d -name "*rosbag2*" -printf "%f\n" | sort)
    if [[ "${#rosbag_dirs[@]}" -ne 2 ]]; then
        echo "ERROR: expected 2 rosbag dirs in $exp, found ${#rosbag_dirs[@]}"
        return 1
    fi
    echo "[$exp] starting (workers=$workers)"
    python3 ros2_exporter.py \
        "$exp/${rosbag_dirs[0]}" \
        "$exp/${rosbag_dirs[1]}" \
        --workers "$workers"
}
